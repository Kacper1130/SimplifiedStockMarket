...
class WalletIntegrationTest {
    ...
    @Test
    void getWallet_whenWalletNotFound_shouldReturn404() throws Exception {
        mockMvc.perform(get("/wallets/nonexistent"))
                .andExpect(status().isNotFound());
    }

    @Test
    void getWallet_shouldReturnWalletState() throws Exception {
    ...
    @Test
    void getStockQuantity_shouldReturnCorrectQuantity() throws Exception {
        mockMvc.perform(post("/wallets/wallet1/stocks/NVDA")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new StockOperation(StockOperationType.BUY))))
                .andExpect(status().isOk());

        mockMvc.perform(get("/wallets/wallet1/stocks/NVDA"))
                .andExpect(status().isOk())
